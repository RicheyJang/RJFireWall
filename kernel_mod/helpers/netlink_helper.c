...
int nlSend(unsigned int pid, void *data, unsigned int len) {
	...
	if (skb == NULL) {
		printk(KERN_WARNING "[fw netfilter] alloc reply nlmsg skb failed!\n");
		return -1;
	}
	...
	printk("[fw netfilter] send to user pid=%d,len=%d,ret=%d\n", pid, nlh->nlmsg_len - NLMSG_SPACE(0), retval);
	return retval;
}
...
void nlRecv(struct sk_buff *skb) {
	...
	if ((nlh->nlmsg_len < NLMSG_HDRLEN) || (skb->len < nlh->nlmsg_len)) {
		printk(KERN_WARNING "[fw netfilter] Illegal netlink packet!\n");
		return;
	}
    // deal data
	data = NLMSG_DATA(nlh);
	...
	printk("[fw netfilter] data receive from user: user_pid = %d, len=%d\n", pid, len);
...
struct sock *netlinkInit() {
	...
	if (!nlsk) {
		printk(KERN_WARNING "[fw netfilter] can not create a netlink socket\n");
		return NULL;
	}
	printk("[fw netfilter] netlink_kernel_create() success, nlsk = %p\n", nlsk);
    return nlsk;
}
